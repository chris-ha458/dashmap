...
/// DashCounter is a thin wrapper around [`DashMap`] using `()` as the value type. It uses
/// methods and types which are more convenient to work with on a set.
///
/// [`DashMap`]: struct.DashMap.html
pub struct DashCounter<K, S = RandomState> {
    pub(crate) inner: DashMap<K, (), S>,
}

impl<K: Eq + Hash + fmt::Debug, S: BuildHasher + Clone> fmt::Debug for DashCounter<K, S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Debug::fmt(&self.inner, f)
    }
}

impl<K: Eq + Hash + Clone, S: Clone> Clone for DashCounter<K, S> {
    fn clone(&self) -> Self {
        Self {
...
impl<K, S> Default for DashCounter<K, S>
where
    K: Eq + Hash,
...
impl<'a, K: 'a + Eq + Hash> DashCounter<K, RandomState> {
    /// Creates a new DashCounter with a capacity of 0.
    ///
    /// # Examples
    ///
    /// ```
    /// use dashmap::DashCounter;
    ///
    /// let games = DashCounter::new();
    ...
    /// use dashmap::DashCounter;
    ///
    /// let numbers = DashCounter::with_capacity(2);
    /// numbers.insert(2);
    /// numbers.insert(8);
    /// ```
...
impl<'a, K: 'a + Eq + Hash, S: BuildHasher + Clone> DashCounter<K, S> {
    ...
    /// let games = DashCounter::with_hasher(s);
    /// games.insert("Veloren");
    /// ```
    ...
    ///
    /// let s = RandomState::new();
    /// let numbers = DashCounter::with_capacity_and_hasher(2, s);
    /// numbers.insert(2);
    /// numbers.insert(8);
    /// ```
    ...
    cfg_if! {
        if #[cfg(feature = "raw-api")] {
            ...
            /// use dashmap::DashCounter;
            ///
            /// let set: DashCounter<i32> = DashCounter::new();
            /// let key = "key";
    ...
    /// use dashmap::DashCounter;
    ///
    /// let soccer_team = DashCounter::new();
...
impl<K: Eq + Hash, S: BuildHasher + Clone> IntoIterator for DashCounter<K, S> {
    type Item = K;
...
impl<K: Eq + Hash, S: BuildHasher + Clone> Extend<K> for DashCounter<K, S> {
...
impl<K: Eq + Hash, S: BuildHasher + Clone + Default> FromIterator<K> for DashCounter<K, S> {
    fn from_iter<I: IntoIterator<Item = K>>(iter: I) -> Self {
        let mut set = DashCounter::default();
